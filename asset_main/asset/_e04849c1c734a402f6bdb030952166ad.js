...
var alt_images_dir = "/shop/skin/crzybllysx/images";var lbl_no_items_have_been_selected = 'No items have been selected';
var current_area = 'C';
...
var txt_ajax_error_note = 'An error occurred while processing a request. Please <a href=\"javascript:void(0);\" onclick=\"javascript:window.location.reload();\">refresh the page.<a><br /><br />If the problem still persists after refreshing the page please <a href=\"javascript:void(0);\" onclick=\"javascript:self.location=\'/contact-us/\'\">contact us<\/a> on the matter.';
 ...
 * @version    $Id: common.js,v 1.13.2.6 2011/07/04 10:44:36 aim Exp $
    ...
    xhr = $.ajax(
      {
        ...
        complete: function(res, status) {
          ...
          //alert(status + ' ' + res.responseText + ' res: ' + res);
          ...
          if ($.browser.msie && parseInt($.browser.version, 10) <= 8 && elm.get(0).id == 'opc_shipping')
...
$.getJSON('http://www.crazybillys.net/dob/checkvisitor.php', function(data){
  //alert('Your ip is: ' +  data.ip);
  //alert('Showpop: ' +  data.showpop);
  if(data.showpop == 1){

//$("#cb-overlay").html(string);
$('<div id="cb-overlay"></div><div id="age-wrapper"><div id="agepop"><form id="ageform" action="" method="post"><input type="hidden" name="submitted" value="1" /><div style="padding:30px 0 0 0;text-align:center;"><p id="message1" style="font-size:17px;line-height:18px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#DB120D;padding:0;margin:0 0 15px 0;">You must be at least 21 years of age to buy wine &amp; spirits.</p><p style="font-size:17px;line-height:18px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#fff;padding:0;margin:0 0 15px 0;">Month <input class="field" style="width:20px;border:none;margin:0 6px 0 3px;padding:0 1px;text-align:center;font-size:17px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#000;" type="text" value="" name="month" maxlength="2" /> Day  <input class="field" style="width:25px;border:none;margin:0 6px 0 3px;padding:0 1px;text-align:center;font-size:17px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#000;" type="text" value="" name="day" maxlength="2" /> Year <input class="field" style="width:45px;border:none;margin:0 0 0 3px;padding:0 1px;text-align:center;font-size:17px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#000;" type="text" value="" name="year" maxlength="4" /></p><p style="font-size:15px;line-height:18px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#DB120D;padding:0;margin:0 0 8px 0;">Enter your date of birth above.</p><p style="padding:0;margin:0 0 10px 0;"><input type="image" id="agesubmit" src="/dob/images/start-shopping.jpg" value="Start Shopping" name="submit" /></p><p style="font-size:12px;line-height:18px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;color:#fff;padding:0;margin:0;">Please do not enter this site if you are not legal purchase age for alcohol.</p></div></form></div></div>').appendTo(document.body);
...
$("#ageform").bind("submit", function() {
	...
	$.ajax({
		...
		success: function(data) {
			 if(data.error == 0){
				 $("#cb-overlay,#age-wrapper").remove();
			 }
			 else if(data.error == 1){
				 $("p#message1").text("Error: You must be at least 21 years of age to buy wine & spirits.");
			 }
		}
	});
...
/*$("#ageform input.field").click(function(){
